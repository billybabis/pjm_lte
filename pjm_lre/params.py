...
class ModelParams:
    ...
    reg_price_share: float = 0.01    # regulation price = 1% of p_ref             -> 0.304 $/MW-h
    ...
    daylight_fixed_hours: tuple = (6, 18)    # local hours [start, end) for "fixed_hours"
    ...
    # a_r markdown ($/MWh of contracted energy).  "appendix" uses the printed values
    # (0.0015, 0.0018) which only cover the regulation (psi_A) term; "regulation" recomputes
    # the same thing from the coefficients; "full" also includes the flexible-reserve
    # (psi_E) term, which is ~200x larger.  See README.
    markdown_mode: str = "appendix"          # "appendix" | "regulation" | "full"
